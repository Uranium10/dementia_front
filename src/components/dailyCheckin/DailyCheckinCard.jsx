...
export default function DailyCheckinCard({ session }) {
  ...
  if (status === 'loading' || status === 'idle') {
    return (
      <div className="bg-slate-200 animate-pulse rounded-3xl h-[160px] w-full" />
    );
  }
  ...
  if (status === 'done' && todayCheckin) {
    const toneInfo = TONE_MAP[todayCheckin.tone] || TONE_MAP['neutral'];
    
    // 시간 표시 
    // 임시 날짜 문자열(YYYY-MM-DD 등)을 적절히 파싱. 백엔드가 주는 형식에 따라 다를 수 있으나 단순 노출.
    const timeString = todayCheckin.checkin_date || "오늘";
